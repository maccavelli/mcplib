...
| 2026-09-14 | 10 post-rollout | The push gate proved that pre-existing `TestMain_RunConfigure` writes `configure --yes --provider=gemini --model=test` to the live platform config. The test file was untouched by Phase 10 and last changed in `9f887d3`; a controlled rerun changed the live config mtime and reproduced the real path | Isolate `HOME`, `USERPROFILE`, `XDG_CONFIG_HOME`, `APPDATA`, and `LOCALAPPDATA` under `t.TempDir()`, pass an explicit fake API key, and assert the resolved config path and written file remain inside that directory. Prove the assertion against a scratch mutation under an externally isolated environment, and guard the real config hash/mtime during the corrected test. No MADR amendment: this is test isolation, not an authentication decision | `main_extra2_test.go` |
